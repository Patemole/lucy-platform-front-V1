...
const Dashboard_teacher_template: React.FC = () => {
  ...
  // URL statique du graphique
  const graphUrl = "http://localhost:5001/static/cluster_plot.html";
...
/*
  useEffect(() => {
    const fetchGraphUrl = async () => {
      try {
        const response = await fetch(`${config.server_url}/cluster/get_graph`, {
          method: 'GET',
          headers: {
        ...
        if (response.ok && result.base_url && result.file_url_part1 && result.file_url_part2 && result.file_url_part3) {
          // Réassembler l'URL à partir des différentes parties
          const fullUrl = `${config.server_url}${result.base_url}${result.file_url_part1}${result.file_url_part2}${result.file_url_part3}`;
          console.log('Full URL:', fullUrl);
          setGraphUrl(fullUrl);
        } else {
          console.error('Failed to get graph URL:', result.error);
        }
      } catch (error) {
...
*/
  ...
  useEffect(() => {
    const fetchPublishedFiles = async () => {
      try {
        const response = await fetch(`${config.server_url}/files/fetching/${course_id}`, {
          method: 'GET',
          headers: {
  ...
  useEffect(() => {
    const fetchGraphUrl = async () => {
      try {
        ...
        const result = await response.json();
        if (response.ok && result.url_cluster) {
          setGraphUrl(`${config.server_url}${result.url_cluster}`);
        } else {
          console.error('Failed to get graph URL:', result.error);
        }
      } catch (error) {
  ...
  */
  ...
  const sendFileToBackend = async (file: File, uid: string, course_id: string) => {
    ...
    const response = await fetch(`${config.server_url}/file/upload_file_from_teacher_dashboard`, {
        method: 'POST',
        body: formData
    });
...
/*
//MODIFICATION AVEC UN ENDPOINT POUR LE GRPAHE 
import React, { useState, useRef, useEffect } from 'react';
import { useTheme } from '@mui/material/styles';
import {
  ThemeProvider, Button, Menu, MenuItem, Snackbar, Box, Typography, Grid,
...
const Dashboard_teacher_template: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPublishedFiles = async () => {
      try {
        //const response = await fetch(`${config.server_url}/file/fetching/${course_id}`, {
        //const response = await fetch(`${config.server_url}/file/fetching/${course_id}`, {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (result.error) {
            ...
            setGraphUrl(result.url);  // Utilisez l'URL renvoyée par l'endpoint
          }
        } else {
          console.error('Failed to get graph URL');
        }
      } catch (error) {
        console.error('Error fetching graph URL:', error);
      }
...
const Dashboard_teacher_template: React.FC = () => {
  ...
  const graphUrl = `${config.server_url}/cluster/get_graph`;
...
/*
//ANCIEN CODE SANS LES URL DYNAMIQUES DE PRODUCTION
//FONCTION UNIQUEMENT  L'UPLOAD LE NOUVEAU DESIGN
import React, { useState, useRef, useEffect } from 'react';
import { useTheme } from '@mui/material/styles';
import {
  ThemeProvider, Button, Menu, MenuItem, Snackbar, Box, Typography, Grid,
