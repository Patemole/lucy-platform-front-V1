//NOUVEAU CODE POUR CRÉER LE COURS ACADEMIC ADVISOR SI IL EXISTE PAS 
...
import { doc, updateDoc, setDoc, getDoc, arrayUnion, query, collection, where, getDocs } from "firebase/firestore";
...
const createAcademicAdvisorCourse = async () => {
  const courseCode = "Academic Advisor";
  const courseName = "Academic Advisor";
  const faculty = "Upenn";
  ...
  const q = query(collection(db, "courses"), where("code", "==", courseCode), where("faculty", "==", faculty));
  ...
  if (querySnapshot.empty) {
    ...
    console.log("Academic Advisor course created.");
  } else {
    console.log("Academic Advisor course already exists.");
  }
};
  ...
  const handleSubmit = async (event) => { 
    ...
    if (Object.keys(newErrors).length > 0) {
      ...
      try {
        ...
        const response = await fetch(`${config.server_url}/files/create-pinecone-index`, {
          method: 'POST',
          headers: {
        ...
        // Create the "Academic Advisor" course if it does not exist
        await createAcademicAdvisorCourse();
...
/*
//ANCIEN CODE SANS LES URL DYNAMIQUES DE PRODUCTION - DERNIER CODE QUI FONCTIONNE
import * as React from 'react';
import Avatar from '@mui/material/Avatar';
import CssBaseline from '@mui/material/CssBaseline';
import Grid from '@mui/material/Grid';
  ...
  const handleSubmit = async (event) => { 
    ...
    if (Object.keys(newErrors).length > 0) {
      ...
      try {
        ...
        // Créer l'index Pinecone via un endpoint
        const response = await fetch(`${config.server_url}/file/api/create-pinecone-index`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
