...
import { AnswerDocument, AnswerPiecePacket, AnswerDocumentPacket, StreamingError, AnswerImage, AnswerTAK, AnswerWaiting } from "../interfaces/interfaces";
...
// Hoverable component used inside AIMessage
...
// AIMessage component definition
...
const Dashboard_eleve_template: React.FC = () => {
  ...
  const fetchCourseOptionsAndChatSessions = async () => {
    if (uid) {
      ...
      if (userSnap.exists()) {
        ...
        const validCourses = courses.filter((course): course is any => course !== null);
  ...
  const handleSendMessageSocraticLangGraph = () => {
    ...
    const newMessage: any = { id: Date.now(), type: "human", content: inputValue };
    setMessages((prevMessages) => [...prevMessages, newMessage]);

    const loadingMessage: any = { id: Date.now() + 1, type: "ai", content: "", personaName: "Lucy" };
  ...
  const onSubmit = async (messageHistory: any[], inputValue: string) => {
    setIsStreaming(true);
    let answer = "";
    let answerDocuments: any[] = [];
    let answerImages: { image_id: string; image_url: string; image_description?: string }[] = [];
    let answerTAK: AnswerTAK[] = [];
    let answerWaiting: string[] = [];
    ...
    try {
      ...
      for await (const packetBunch of sendMessageFakeDemo({
        ...
        setMessages((prevMessages) => {
          ...
          console.log("TAK before set:", flattenedTAK);
          console.log("Waiting messages before set:", answerWaiting);
          ...
          updatedMessages[lastMessageIndex] = {
            ...
            TAK: flattenedTAK ?? [],
            waitingMessages: answerWaiting ?? [],
          };

          console.log("Updated message:", updatedMessages[lastMessageIndex]);
...
// FICHIER QUI FONCTIONNE POUR LA DÉMO MAIS SANS TAK ET SANS WAITING ANSWER
