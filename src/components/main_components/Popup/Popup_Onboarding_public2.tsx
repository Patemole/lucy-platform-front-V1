    ...
    <div className="fixed top-0 left-0 w-full h-full flex justify-center items-center bg-black bg-opacity-50 p-4">
      ...
      >
        ...
        <div className="flex items-center justify-center space-x-3 flex-wrap">
          <h2 className="text-lg sm:text-2xl font-bold">Meet <span className="text-purple-600">Social Thread</span></h2>
          {unreadCount > 0 && (
            <div className="bg-red-500 text-white rounded-md px-2 py-1 text-sm font-medium flex justify-center items-center min-w-[24px] h-[24px]">
              {unreadCount}
            </div>
          )}
        </div>
        ...
        <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 text-left">
          {messageTypes.map(({ type, description, color }) => (
            <div key={type}>
              <h3 className="text-sm sm:text-lg font-semibold">{type === "Public" ? "On Public" : "On Private"}</h3>
              <p className="text-gray-700 text-sm sm:text-base">{description}</p>
              <div
                className="flex items-center justify-center px-4 py-2 mt-2 rounded-md font-medium"
                style={{
