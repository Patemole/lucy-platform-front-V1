...
/* ANCIEN CODE QUI MARCHE (23/09/2024)
...
//NOUVEAU CODE POUR LA SLIDEBAR DE SELECTION DE CRÉNEAU
// src/components/CourseSelectionChat.tsx

import React, { useState, useEffect, useRef, KeyboardEvent } from 'react';
import { Box, TextField, IconButton, Avatar, Snackbar } from '@mui/material';
import SendIcon from '@mui/icons-material/Send';
import AIMessage from '../components/CourseSelectionAIMessages';
import Calendar from '../components/NewCalendarCustom'; // Ensure Calendar is imported
import {
  Message,
  AnswerTAK,
  AnswerCourse,
...
interface ChatProps {
  handleAddCourseToCalendar: (selectedSlot: CourseSlot, answercourse: AnswerCourse) => void;  // Accept two parameters here
  handleOpenDrawer: (course: AnswerCourse) => void; // New prop to open Drawer
}

const Chat: React.FC<ChatProps> = ({ handleAddCourseToCalendar, handleOpenDrawer }) => {
  const theme = useTheme();
  ...
  const [snackbarOpen, setSnackbarOpen] = useState(false);
  const scrollableDivRef = useRef<HTMLDivElement>(null);
  // State to store selected course slot
  const [newCourseSlot, setNewCourseSlot] = useState<{ slot: CourseSlot; title: string} | null>(null);
            ...
            {message.type === 'human' ? (
              ...
              >
                <AIMessage
                  ...
                  handleAddCourseToCalendar={handleAddCourseToCalendar}
                  handleShowSnackbar={handleShowSnackbar}
                  handleOpenDrawer={handleOpenDrawer} // Pass the handler
                />
              </Box>
            )}
          </Box>
